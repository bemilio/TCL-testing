...
%     [epsilon*ones(r-1, 1); 1-(r-1)*epsilon]';
    [epsilon*ones(r-2, 1); 1-(r-1)*epsilon; epsilon]';
...
%     "uniform";
%     "bottom right delta"
    "bottom right controllable delta"
%     "top right delta";
%     "bottom left delta"
    ];
...
for rho0_iterations= 1:size(initial_conditions, 2)
    ...
    [ top_limit_LP, bottom_limit_LP ] = calculateTubeLP(rho0, N, P, B); 
    for reference_iterations = 1:1
        for k = 1:N
            P_track(k) = ((top_limit(k)+bottom_limit(k)) /2 ) + ((top_limit(k)-bottom_limit(k))/2)*sin(0.5 * reference_iterations*k);
        end
                ...
                for j = 1:n
                    if B(i,j)==0
                        if P(i,j) == 0
                            ...
                            elseif (j==1 + n/2 || j==n/2)
                                if(i == j)
                                    index_complementary = find(and(P(:,j) > 0,  (1:n)'~= j));
                                    
                                    Constraints = [Constraints; M(i,j,k)== (rho(j, k) - M(index_complementary, j, k))];
                                else
